...
static int xpcie_probe(struct pci_dev *pci, const struct pci_device_id *pci_id)
{
    ...
    tmp = pci_alloc_irq_vectors(dev->pciDev, 1, 32, PCI_IRQ_MSI);
    if (tmp < 0) {
    ...
    /*dev->h2dBuffer = dma_alloc_coherent(&pci->dev, DMA_BUF_SIZE, &dev->h2dAddr,GFP_KERNEL);*/
    if (!dev->h2dBuffer) {
    ...
    /*dev->d2hBuffer = dma_alloc_coherent(&pci->dev, DMA_BUF_SIZE, &dev->d2hAddr,GFP_KERNEL);*/
    if (!dev->d2hBuffer) {
