...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < TRANSFER_SIZE; ++i) {
		writeBuffer[i] = 0xDEADBEAD;
	}

	for (i = 0; i < TRANSFER_SIZE; ++i) {
		readBuffer[i] = 0xBEADDEAD;
	}
	...
	readTLPSize = 1;
	readTLPCount = TRANSFER_SIZE/maxPayloadSize;
	...
	} while (((dmaControlRead & 0x1111) != 0x0101) && timeout--);
    ...
    if (ioctl(fd, XBMD_IOC_SET_WRITE_FLAG, 0) < 0) {
        printf("IOCTL failed setting write flag\n");
        return -1;
    }
    read_data(fd, readTLPSize * readTLPCount * sizeof(uint32_t), readBuffer);
	...
	if (readEnable) {
		for (i = 0; i < (readTLPSize * readTLPCount); ++i) {
			readData = readBuffer[i];
            printf("*************** file : %s, func : %s, line : %d readData = 0x%08x****************\n", __FILE__, __func__, __LINE__,readData);
			if (readData != TestPattern) {
				printf("Mismatch: index %d, wrote value : 0x%08x, got value: 0x%8x \n", i, TestPattern, readData);
			}
		}
