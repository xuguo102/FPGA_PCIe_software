...
int main(int argc, char **argv)
{
    ...
    // Do DMA Write
    if (ioctl(fd, XBMD_IOC_WRITE_WR_PTRN, TestPattern) < 0) {
	...
	} while (((dmaControlWrite & 0x1111) != 0x0101) && timeout--);
	...
	if (ioctl(fd, XBMD_IOC_SET_WRITE_FLAG, 1) < 0) {
