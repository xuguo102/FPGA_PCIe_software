...
int main(int argc, char **argv)
{
	...
	writeBuffer = (uint32_t*)malloc(sizeof(uint32_t) * 1024);
	readBuffer  = (uint32_t*)malloc(sizeof(uint32_t) * 1024);
	...
	writeTLPCount = 1024/maxPayloadSize;
	readTLPSize = maxPayloadSize;
	readTLPCount = 1024/maxPayloadSize;
	...
	readEnable = doRead ? 0 : 1;
	if (readEnable) {
		printf("*************** readEnable ****************\n");
	}
	writeEnable = doRead ? 1 : 0;
	if (writeEnable) {
		printf("*************** writeEnable ****************\n");
	}
